# 0 "fir.c"
# 1 "/home/ykhsieh/SoCDesignLab/caravel-soc_fpga-lab_new/lab-caravel_fir/testbench/counter_la_fir//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "fir.c"
# 1 "fir.h" 1
 ...
 *((volatile uint32_t*) 0x30000010) = 64;
 volatile uint32_t* p = ((volatile uint32_t*) 0x30000040);
 for(int i = 0 ; i < 11 ; i ++){
  *(p++) = taps[i];
 }
...
int* __attribute__ ( ( section ( ".mprjram" ) ) ) fir(){
  ...
  *((volatile uint32_t*) 0x30000080) = i;
  ...
  (*(volatile uint32_t*)0x2600000c) = 0x005A0000 + ((outputsignal[64 -1] & 0x000000FF) << 24);


 for(int i = 0; i < 64; i++) {
     (*(volatile uint32_t*)0x2600000c) = outputsignal[i] << 16;
  }
